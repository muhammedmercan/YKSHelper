...
public class RobotActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private String checkData(String queryString) {
        ...
        int counter = 0;
        for (int i = 0; i < multiAutoCompleteTextViewDepartmentText.length -1; i++) {

            counter +=1;


            if (i == multiAutoCompleteTextViewDepartmentText.length -2) {
                queryString = queryString + "department = '" + multiAutoCompleteTextViewDepartmentText[i].trim() + "') ";

            }
            else {
                queryString = queryString + "department = '" + multiAutoCompleteTextViewDepartmentText[i].trim() + "' OR ";
            }}



        for (int i = 0; i < multiAutoCompleteTextViewCityText.length -1; i++) {

            if(counter != 0) {
                queryString = queryString + "AND (";
            }

            if (i == multiAutoCompleteTextViewCityText.length -2) {
                queryString = queryString + "city = '" + multiAutoCompleteTextViewCityText[i].trim() + "') ";

            }
            else {
                queryString = queryString + "city = '" + multiAutoCompleteTextViewCityText[i].trim() + "' OR ";

            }
            counter+=1;
        }

        for (int i = 0; i < multiAutoCompleteTextViewUniversityText.length -1; i++) {

            if(counter != 0) {
                queryString = queryString + "AND (";
            }

            if (i == multiAutoCompleteTextViewUniversityText.length -2) {
                queryString = queryString + "university = '" + multiAutoCompleteTextViewUniversityText[i].trim() + "') ";

            }
            else {
                queryString = queryString + "university = '" + multiAutoCompleteTextViewUniversityText[i].trim() + "' OR ";
            }
            counter+=1;
        }
        ...
        if (!checkBoxNew.isChecked()) {
            ...
            queryString = queryString + "status != 'Yeni')";
            counter += 1;
        }
        ...
        if (!checkBoxNotFilled.isChecked()) {
            ...
            queryString = queryString + "status != 'Dolmadı')";
            counter += 1;
        }
